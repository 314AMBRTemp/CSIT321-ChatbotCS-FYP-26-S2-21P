...
**Identifiers:**
- Project ID: `csci321-chatbot-for-cs`
- Agent ID: `e7e8fe80-878e-484e-b3d7-76ff1222b47b`
- Location: `asia-southeast1`
- Webhook: `verify_and_fetch_order`
- Webhook secret token: `6e037887676fca2fe030065b2abc127a91ca899062295c2e58f58d3659f22c27`
...
**Intent library:**
- `main.*` — track_package, change_delivery_address, cancel_order, report_missing_package, report_damaged_package, return_item, authenticated_track, create_ticket, restart
...
| Flow | Status | Pages |
|---|---|---|
| Track package (unauthenticated) | ✅ Complete | Start → Verify Identity → Confirm Details → Track Result Page |
| Track package (authenticated) | ✅ Complete | Start → Authenticated Track Result (webhook: authenticated_track) |
| Change delivery address | ✅ Complete | Start → Verify Identity → Collect New Address → Confirm Address Update → Address Update Result |
| Cancel order | ✅ Complete | Start → Verify Identity → Confirm Details → Confirm Cancellation → Cancel Result |

**flow_intent parameter:**
Each flow sets `flow_intent` on the Start Page route so Verify Identity knows where to transition after collecting credentials:
- `track_package` → Confirm Details
- `change_delivery_address` → Collect New Address
- `cancel_order` → Confirm Details (then routes to Confirm Cancellation)

**Webhook tags and handlers:**

| Tag | What it does |
|---|---|
| `track_package` | Unauthenticated order lookup — verifies order_id + email against DB |
| `authenticated_track` | Logged-in lookup — skips identity verification, handles single/multiple active orders |
| `change_delivery_address` | Updates delivery_address on order, creates address_change ticket |
| `cancel_order` | Sets order status to Cancelled, creates cancellation ticket |
| `create_ticket` | Creates a support ticket with issue_type and description |
| `faq_search` | Fuzzy-matches user query against BotResponse table (fuzzywuzzy, threshold 60%) |

---

## Known Bugs / Watch Out For

- `BotResponse` was missing from the import in `webhook.py` — must be imported from `models`
- `serviceAccountKey.json` is gitignored and must be manually placed in project root
- `static/css/main.css` references `oTx.jpg` but file is `oTx.webp` — gradient fallback shows fine
- `ngrok.exe` is gitignored — must be manually placed in root if needed
- `*.db` files are gitignored — run `seed.py`, `seed_admin.py`, `seed_more.py` after fresh clone
- The `1.ref-storefront/` folder is read-only reference — do not modify
- `LOCATION` in `webhook.py` must be `asia-southeast1` not `global` — agent is in asia-southeast1
- After each ngrok restart, update webhook URL in Dialogflow CX: Manage > Webhooks > verify_and_fetch_order
...
| Item | Notes |
|---|---|
| Phase 4 — Email confirmation | Gmail SMTP on address change, cancellation, ticket creation |
| Phase 5 — Admin dashboard enhancements | Ticket filtering, email trigger from dashboard |
| Phase 6 — Deployment | Railway or Render, .env secrets, permanent URL, optional PostgreSQL |
| Chat widget login prompt | Show login button in widget for unauthenticated users |
| Rate limiting | Flask-Limiter on /chat endpoint |
| .env for all secrets | python-dotenv installed, .env file needs creating |

---

## GitHub Remote
`https://github.com/314AMBRTemp/CSIT321-ChatbotCS-FYP-26-S2-21P.git`
Main branch (`main`) holds the production-ready version.
`120626-project-RMH` is the active development version with extended features.
